function Draw_Time_And_Try_Times(result_tb)
%DRAW_TIME_AND_TRY_TIMES Plot lines showing try times and CPU time
...
try_times_catagory = unique(result_tb.Try);
...
for i = 1:length(size_catagory)
    ...
    data = zeros(1, length(try_times_catagory));
    for j = 1:length(try_times_catagory)
        try_times = try_times_catagory(j);
        temp_tb = sub_tb(sub_tb.Try == try_times, :);
...
for i = 1:length(data_for_fig)
    plot(try_times_catagory, data_for_fig{i}, ...
        LineWidth = 1.5, ...
        Color = color_set(i, :), ...
        Marker = marker_set(i), ...
